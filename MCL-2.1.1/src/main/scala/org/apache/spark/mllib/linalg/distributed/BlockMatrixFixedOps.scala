      ...
      val newBlocks = flatA.cogroup(flatB, resultPartitioner).flatMap { case (pId, (a, b)) =>
        ...
        a.flatMap { case (leftRowIndex, leftColIndex, leftBlock) =>
          ...
          matrixHelper.getOrElse(leftColIndex, List()).map { case (rightRowIndex, rightColIndex, rightBlock) =>
            ...
            if (hitCount % math.max(1, hits / 10) == 0) {
              if (hits==0) logInfo(s"calculated $hitCount multiplications - hits ZERO (${hitCount * 100}%)")
              else logInfo(s"calculated $hitCount multiplications (${hitCount * 100 / hits}%)")
            }
            hitCount += 1
...
def toIndexedRowMatrixNew(me:BlockMatrix): IndexedRowMatrix = {
    val cols = me.numCols().toInt

    require(cols < Int.MaxValue, s"The number of columns should be less than Int.MaxValue ($cols).")

    val rows = me.blocks.flatMap { case ((blockRowIdx, blockColIdx), mat) =>
      mat.rowIter.zipWithIndex.map {
        case (vector, rowIdx) =>
          blockRowIdx * me.rowsPerBlock + rowIdx -> (blockColIdx, vector)
      }
      ...
      val wholeVector =
        ...
        val wholeVectorBuf = VectorBuilder.zeros[Double](cols)
          vectors.foreach { case (blockColIdx: Int, vec: Vector) =>
            val offset = me.colsPerBlock * blockColIdx
          ...
          vectors.foreach { case (blockColIdx: Int, vec: Vector) =>
            val offset = me.colsPerBlock * blockColIdx
            wholeVectorBuf(offset until Math.min(cols, offset + me.colsPerBlock)) := vec.asBreeze
          }
